...
class Moustache():
    ...
    def calculate_averaged_center(mask):
        xy = sum([mask[FaceIndexes.NoseTip],
                  mask[FaceIndexes.MouthTopCenter],
                  mask[FaceIndexes.NoseUnder]]) / 3.0
    ...
    def set_goal(self, frames):
        total_distance = np.subtract(self.finish_center, self.start_center)

        center_delta = np.divide(total_distance, frames)
    ...
    def draw_moustache(self, out, cycle):
        ...
        for c in range(0, 3):
            a = alpha_s * rotated[:, :, c]
            b = alpha_l * out[y1:y2, x1:x2, c]
            out[y1:y2, x1:x2, c] = (a + b)
